...
static node_t* travelList(int index)
{
    ...
    if (index <-1)
    {
        printf("[ERROR] either list is null or index is wrong! (offset : %i)\n", index);
        return NULL;
    }
    if (index >= list_cnt)
    {
        printf("[ERROR] index is larger than length (len:%i, index:%i)\n", list_cnt, index);
        return NULL;
    }
...
int trvldb_addTail(void* obj)
{
    ...
    if (obj == NULL)
    {
        printf("[ERROR] Failed to do addTail : input object indicates NULL!\n");
        return -1;
    }
...
*/
int trvldb_deleteData(int index)
{
    node_t* ndPrevPtr;
    node_t* ndNextPtr;
    node_t* delNdPtr;
    ...
    //parameter checking
    if ( (delNdPtr = travelList(index)) == NULL)
    {
        printf("[ERROR] Failed to do deleteData : input index is invalid (%i)!\n", index);
        return -1;
    }
...
\*/
void* trvldb_getData(int index)
{
    void* obj = NULL;
    node_t* ndPtr;
    
    //parameter checking
    if ((ndPtr = travelList(index)) != NULL)
    {
        obj = (void*)ndPtr->obj;
        listPtr = ndPtr;
    }
    ...
    if (obj == NULL)
        printf("ERROR!! there is no data of index %i\n", index);
